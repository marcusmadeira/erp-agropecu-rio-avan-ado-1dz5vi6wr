...
const schema = z.object({
  ...
  peso_kg: z.coerce.number().positive('Peso deve ser maior que zero'),
...
export default function PesagemForm({ isOpen, onClose, initialData, animais, onSubmit }: Props) {
    ...
    if (isOpen) {
      if (initialData) {
        form.reset({
          ...
          data_pesagem: parseISO(initialData.data_pesagem),
          peso_kg: initialData.peso_kg,
          responsavel_pesagem: initialData.responsavel_pesagem || '',
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[500px] border-0 p-0 overflow-hidden">
        <DialogHeader className="bg-slate-900 text-white p-6 pb-4">
          <DialogTitle>{initialData ? 'Editar Pesagem' : 'Nova Pesagem'}</DialogTitle>
        </DialogHeader>
        <Form {...form}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      placeholder="Notas adicionais sobre a pesagem..."
                      {...field}
