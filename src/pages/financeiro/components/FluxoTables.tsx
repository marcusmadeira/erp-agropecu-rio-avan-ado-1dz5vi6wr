...
export function FluxoTables({ upcoming, overdue }: { upcoming: any[]; overdue: any[] }) {
  ...
  const wppLink = (cliente: any, valor: number, msg: string) => {
    ...
    const text = encodeURIComponent(
      `Olá ${cliente?.nome_razao_social || 'Cliente'}, ${msg} ${fValor}.`,
    )
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
            ...
            <TableBody>
              {upcoming.slice(0, 10).map((p) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell>
                    {p.expand?.venda_id?.expand?.cliente_id?.nome_razao_social || '-'}
                  </TableCell>
                  ...
                  <TableCell>
                    <Button
                      ...
                      onClick={() =>
                        wppLink(
                          p.expand?.venda_id?.expand?.cliente_id,
                          p.valor_parcela,
                          'lembrete de vencimento no valor de',
                        )
                      }
                    >
      ...
      <Card>
        <CardHeader>
          <CardTitle>Inadimplentes (&gt; 5 dias)</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {overdue.slice(0, 10).map((p) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell>
                    <Badge variant="destructive">{p.dias_atraso} dias</Badge>
                  </TableCell>
                  <TableCell>R$ {p.valor_parcela.toFixed(2)}</TableCell>
                    ...
                    <Button
                      ...
                      onClick={() =>
                        wppLink(
                          p.expand?.venda_id?.expand?.cliente_id,
                          p.valor_parcela,
                          'notamos uma pendência no valor de',
                        )
                      }
                    >
                      <Phone className="h-4 w-4 text-green-600" />
          ...
          <div className="grid gap-4 py-4">
            <div className="space-y-2">
              ...
              <div className="font-medium">
                {negociarItem?.expand?.venda_id?.expand?.cliente_id?.nome_razao_social}
              </div>
            </div>
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="Detalhes do acordo, nova data prometida, etc."
              />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setNegociarItem(null)}>
              Cancelar
            </Button>
