...
import { getMetas, createMeta, deleteMeta } from '@/services/metas'
...
export default function MetasKPIs() {
  ...
  const loadData = async () => {
    try {
      ...
      const arrobas = animaisVendidos * 18
      const custo = arrobas ? totalDespesas / arrobas : 0
  ...
  const renderDashboardCard = (meta: Meta) => {
    ...
    if (isHigherBetter) {
      progress = (actual / meta.valor_meta) * 100
      isGood = actual >= meta.valor_meta
    } else {
      progress = (meta.valor_meta / actual) * 100
    ...
    return (
      ...
      >
        <CardHeader className="pb-2">
          ...
          <CardDescription>{meta.periodo || 'Sem período'}</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">
                Atual: <strong className="text-foreground">{formatValue(actual)}</strong>
              </span>
              <span className="text-muted-foreground">
                Meta: <strong className="text-foreground">{formatValue(meta.valor_meta)}</strong>
              </span>
            </div>
            <Progress
              value={progress}
            ...
            {!isGood && (
              <p className="text-xs text-red-600 mt-2 bg-red-50 p-2 rounded">
                Atenção: Meta fora do esperado. Ações corretivas podem ser necessárias.
              </p>
            )}
          </div>
        </CardContent>
    ...
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Acompanhamento Real vs Meta da fazenda.</p>
        </div>
      ...
      <Tabs defaultValue="dashboard">
        <TabsList className="mb-4">
          <TabsTrigger value="dashboard">Dashboard (Real vs Meta)</TabsTrigger>
          <TabsTrigger value="gestao">Gestão de Metas</TabsTrigger>
        </TabsList>
                ...
                {metas.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {metas.map((m) => (
                      ...
                      >
                        <div>
                          <p className="font-semibold">{m.tipo_meta}</p>
                        ...
                        <div className="flex items-center gap-4">
                          <p className="font-bold">Alvo: {m.valor_meta}</p>
