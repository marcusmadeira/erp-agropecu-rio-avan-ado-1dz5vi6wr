...
export default function AssistenteIA() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-6rem)] max-h-[850px] bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-white z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900 tracking-tight">Assistente IA</h1>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 bg-slate-50/50 scroll-smooth">
        {messages.length === 0 && !isLoading ? (
          <div className="h-full flex flex-col items-center justify-center text-center space-y-4 animate-fade-in-up">
            <div className="w-20 h-20 bg-white border border-slate-100 shadow-sm rounded-2xl flex items-center justify-center mb-2">
              <Bot className="w-10 h-10 text-slate-900" />
            </div>
            <div>
              <h3 className="text-xl font-bold text-slate-900">Como posso ajudar?</h3>
            ...
            <div className="flex gap-2 mt-4">
              <span className="px-3 py-1 bg-white border border-slate-200 rounded-full text-xs text-slate-500 font-medium shadow-sm">
                Rebanho
              </span>
              <span className="px-3 py-1 bg-white border border-slate-200 rounded-full text-xs text-slate-500 font-medium shadow-sm">
                Financeiro
            ...
            <div key={msg.id} className="space-y-6">
              ...
              {msg.pergunta !== 'Mensagem de boas-vindas' && (
                <div className="flex justify-end gap-4 animate-fade-in-up">
                  <div className="max-w-[75%] bg-slate-900 text-white rounded-2xl rounded-tr-sm px-5 py-3.5 shadow-sm">
              ...
              {msg.resposta && (
                <div className="flex justify-start gap-4 animate-fade-in-up">
                  <div className="w-10 h-10 rounded-full bg-slate-900 flex items-center justify-center shrink-0 border-2 border-white shadow-sm mt-1">
                    <Bot className="w-5 h-5 text-white" />
                  </div>
                  <div className="max-w-[85%] bg-white border border-slate-200 text-slate-800 rounded-2xl rounded-tl-sm px-6 py-4 shadow-sm">
                    <p className="text-[15px] leading-relaxed whitespace-pre-wrap">
        ...
        {isLoading && (
          <div className="flex justify-start gap-4 animate-fade-in-up">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl rounded-tl-sm px-6 py-5 shadow-sm flex items-center gap-2">
      ...
      <div className="p-4 bg-white border-t border-slate-100 z-10">
        <form onSubmit={handleSend} className="relative flex items-center max-w-4xl mx-auto">
          <Input
            ...
            className="w-full pr-14 py-7 text-[15px] bg-slate-50 border-slate-200 rounded-2xl shadow-sm focus-visible:ring-slate-900 focus-visible:bg-white transition-all"
          ...
          <Button
            ...
            className="absolute right-2 h-11 w-11 bg-slate-900 hover:bg-slate-800 text-white rounded-xl transition-all shadow-sm"
          >
