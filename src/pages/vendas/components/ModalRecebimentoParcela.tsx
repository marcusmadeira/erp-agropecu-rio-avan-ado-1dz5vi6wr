...
export function ModalRecebimentoParcela({ open, onOpenChange, parcela, onSuccess }: any) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await pb.collection('parcelas_venda').update(parcela.id, {
        ...
        forma_pagamento: form.forma_recebimento,
      })
      ...
      <DialogContent className="bg-white">
        <DialogHeader>
          <DialogTitle className="text-emerald-900">
            Registrar Baixa Manual - Parcela{' '}
            {parcela?.numero_parcela === 0 ? 'Sinal' : parcela?.numero_parcela}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4 text-black">
          ...
          <div>
            ...
            <Input
              ...
              placeholder={String(parcela?.valor_parcela || '')}
              className="border-gray-300"
            />
          </div>
          <div>
